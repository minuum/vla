...
```python
def update_history_memory(self, new_token):
    """히스토리 메모리 업데이트"""
    self.history_memory.append(new_token)
    
    # 윈도우 크기 초과 시 오래된 토큰 제거
    if len(self.history_memory) > self.history_len:
        self.history_memory.pop(0)
    
    # 히스토리 시퀀스로 변환
    if len(self.history_memory) == self.history_len:
        hist_sequence = torch.cat(self.history_memory, dim=1)
        return hist_sequence
    else:
        return None
```
...
class MLPSigmoidHead(nn.Module):
    """Gripper Action Head (1-DOF)"""
    def __init__(self, input_dim, output_dim):
        super().__init__()
        self.fc = nn.Linear(input_dim, output_dim)
        
    def forward(self, x):
        return torch.sigmoid(self.fc(x))  # [0, 1] 출력
```
...
```python
def huber_loss_advantage(pred, target, delta=1.0):
    """
    Huber Loss의 장점:
    1. MSE보다 이상치에 robust
    2. MAE보다 gradient가 안정적
    3. 로봇 제어에 적합
    """
    diff = pred - target
...
```python
def log_training_metrics(self, loss_dict):
    """학습 메트릭 로깅"""
    self.log('train/loss_arm', loss_dict['loss_arm'])
    self.log('train/loss_gripper', loss_dict['loss_gripper'])
    self.log('train/acc_gripper', loss_dict['acc_gripper'])
    self.log('train/total_loss', loss_dict['loss'])
...
def validation_step(self, batch, batch_idx):
    ...
    with torch.no_grad():
        ...
        # 검증 메트릭 계산
        val_metrics = {
            'val/loss_arm': loss_dict['loss_arm'],
            'val/loss_gripper': loss_dict['loss_gripper'],
            'val/acc_gripper': loss_dict['acc_gripper']
        }
...
- **메모리 효율성**: Mixed precision, 그래디언트 누적
- **학습 안정성**: 그래디언트 클리핑, 낮은 학습률
- **LSTM 특화**: 4층 구조, 히스토리 기반 예측
