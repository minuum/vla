...
def main():
    print("="*70)
    print("  Vision Encoder 메모리 측정 (FP16)")
    print("="*70)
    print()
    ...
    # Vision Encoder 로딩
    print("📦 [2/4] Vision Encoder 로딩 중...")
    start_time = time.time()
    
    try:
        # Kosmos-2의 Vision Model만 로드
        full_model = AutoModel.from_pretrained(
            model_path,
            torch_dtype=torch.float16,
            trust_remote_code=True
        )
        vision_encoder = full_model.vision_model.to('cuda')
    ...
    print(f"   dtype: {next(vision_encoder.parameters()).dtype}")
    print()
    
    # 결과 저장
    result = {
        "model": "Vision Encoder (Kosmos-2)",
        "dtype": "FP16",
        "baseline": baseline,
    ...
    output_file = output_dir / "memory_vision_fp16.json"
    ...
    print()
    print("="*70)
    print("✅ Vision Encoder 메모리 측정 완료!")
    print("="*70)
