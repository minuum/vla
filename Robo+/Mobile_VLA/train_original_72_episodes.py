#!/usr/bin/env python3
"""
원본 72개 데이터셋을 사용한 Simplified RoboVLMs 모델 학습
"""
...
class SimplifiedCLIPModel(nn.Module):
    """Simplified CLIP 기반 모델 (원본 Simplified RoboVLMs와 유사)"""
...
def train_original_72_episodes_model(data_path, output_dir, num_epochs=10, batch_size=4, 
    ...
    # 모델 생성
    logger.info("🤖 모델 생성 중...")
    model = SimplifiedCLIPModel(processor, hidden_dim=512, dropout=0.2)
    ...
    for epoch in range(num_epochs):
        ...
        if val_mae < best_mae:
            best_mae = val_mae
            best_checkpoint_path = output_path / f"best_original_72_episodes_model_epoch_{epoch+1}.pth"
            torch.save({
    ...
    # 최종 모델 저장
    final_checkpoint_path = output_path / "final_original_72_episodes_model.pth"
    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': trainer.optimizer.state_dict(),
        'epoch': epoch + 1,
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='Train Original 72 Episodes Model')
    parser.add_argument('--data_path', type=str, default='mobile_vla_dataset', help='Path to dataset')
    parser.add_argument('--output_dir', type=str, default='original_72_episodes_results', help='Output directory')
    ...
    # 훈련 실행
    model, trainer = train_original_72_episodes_model(
