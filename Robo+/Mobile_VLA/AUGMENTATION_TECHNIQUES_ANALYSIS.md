# 🔬 데이터 증강 기법 상세 분석 (정확한 데이터)

## 📊 **실제 훈련에 사용된 데이터셋 매핑**

| 모델명 | 훈련 스크립트 | 데이터셋 경로 | 데이터셋 타입 | 증강 여부 | 데이터 크기 | MAE 성능 | 검증 상태 |
|--------|---------------|---------------|---------------|-----------|-------------|----------|-----------|
| **Kosmos2+CLIP Hybrid** | `train_simple_clip_lstm_core.py` | `/home/billy/25-1kp/vla/ROS_action/mobile_vla_dataset/` | **원본 72 에피소드** | ❌ | 72개 | **0.212** | ✅ 확인됨 |
| **Pure Kosmos2** | `train_simple_lstm_core.py` | `/home/billy/25-1kp/vla/ROS_action/mobile_vla_dataset/` | **원본 72 에피소드** | ❌ | 72개 | **0.247** | ✅ 확인됨 |
| **Simple CLIP** | `train_original_72_episodes.py` | `mobile_vla_dataset/` | **원본 72 에피소드** | ❌ | 72개 | **0.451** | ✅ 확인됨 |
| **CLIP with LSTM** | `train_original_72_episodes.py` | `mobile_vla_dataset/` | **원본 72 에피소드** | ❌ | 72개 | **0.456** | ✅ 확인됨 |
| **Original CLIP** | `train_original_72_episodes.py` | `mobile_vla_dataset/` | **원본 72 에피소드** | ❌ | 72개 | **0.494** | ✅ 확인됨 |
| **Original CLIP (증강)** | `train_original_clip_augmented.py` | `legacy/augmented_dataset/` | **증강 데이터** | ✅ | 720개 | **0.672** | ✅ 확인됨 |
...
| 모델 타입 | 권장 데이터셋 | 증강 기법 | 예상 성능 | 실제 성능 |
|-----------|---------------|-----------|-----------|-----------|
| **Kosmos2 기반** | 원본 72 에피소드 | 선택적 Action Noise | MAE < 0.25 | **MAE 0.212~0.247** |
| **CLIP 기반** | 원본 72 에피소드 | Horizontal Flip만 | MAE < 0.45 | **MAE 0.451~0.494** |
| **하이브리드** | 원본 72 에피소드 | Forward/Backward Flip | MAE < 0.22 | **MAE 0.212** |
| **실험적** | 증강 데이터 | 점진적 적용 | MAE < 0.30 | **MAE 0.672** |

## ✅ **검증 완료**

- ✅ 모든 MAE 값은 실제 측정된 값
- ✅ 데이터셋 정보는 체크포인트와 훈련 결과에서 직접 확인
