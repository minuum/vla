...
기존 VLA 모델들은 대규모 데이터셋에 의존하는 경향이 있어, 제한된 데이터로도 우수한 성능을 달성할 수 있는 효율적인 학습 방법에 대한 연구가 부족하다. 특히 모바일 로봇 환경에서는 데이터 수집의 어려움으로 인해 효율적인 학습 방법이 더욱 중요하다.

### 3. 제안하는 모바일 VLA 시스템

기존 VLA 모델의 한계점을 해결하기 위해 모바일 환경에 최적화된 VLA 시스템을 제안한다.
...
기존 VLA 모델들이 7-DoF 로봇 팔 제어에 집중한 것과 달리, 본 연구에서는 2-DoF 모바일 로봇 내비게이션에 특화된 시스템을 구현하였다.
