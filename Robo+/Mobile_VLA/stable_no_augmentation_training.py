...
class StableKosmos2Trainer(MobileVLATrainer):
    """안정적인 Kosmos2 트레이너 - 증강 없음"""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.z_weight = 0.05  # Z축 특별 처리 가중치
        self.gradient_clip = 1.0  # 그래디언트 클리핑
        self.learning_rate_scheduler = True  # 학습률 스케줄러 플래그
        
        print("✅ StableKosmos2Trainer 초기화 완료")
        print(f"   Z축 가중치: {self.z_weight}")
        print(f"   데이터 증강: 비활성화")
        print(f"   그래디언트 클리핑: {self.gradient_clip}")
    ...
    def train_step_stable(self, batch):
        """안정적인 학습 스텝 (증강 없음)"""
        try:
            # 액션 처리
...
def stable_training():
    """안정적인 학습 (증강 없음)"""
    print("🚀 안정적인 Kosmos2 학습 시작! (증강 없음)")
    print("=" * 60)
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            try:
                metrics = trainer.train_step_stable(batch)
    ...
    results = {
        ...
        'model_info': {
            'architecture': 'Stable Kosmos2 (No Augmentation)',
    ...
    with open('stable_training_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n💾 결과 저장됨: stable_training_results.json")
