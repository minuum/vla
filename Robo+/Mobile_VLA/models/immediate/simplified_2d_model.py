#!/usr/bin/env python3
"""
🎯 Case 1: 즉시 적용 - 단순화된 2D 모델
목표: MAE 0.8 → 0.5, 정확도 0% → 15%
특징: 모델 구조 단순화 + 기본 데이터 증강
"""
...
class Simplified2DActionModel(nn.Module):
    ...
    def __init__(self, processor, vision_dim=1024, language_dim=2048, action_dim=2, 
        ...
        logger.info(f"🔍 Language adapter initialized with: {language_dim} → {hidden_dim}")
        logger.info(f"🔍 Language adapter weight shape: {self.language_adapter.weight.shape}")
    ...
    def extract_language_features(self, texts):
        ...
        with torch.no_grad():
            if 'input_ids' in inputs:
                ...
                logger.info(f"🔍 Text features shape: {language_features.shape}")
            else:
