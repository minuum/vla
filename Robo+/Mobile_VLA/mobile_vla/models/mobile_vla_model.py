...
class MobileVLAModel(nn.Module):
    ...
    def forward(
        ...
        # RoboVLMs 호환을 위한 액션 및 마스크 변환
        robovlms_actions = self.policy_head.convert_to_robovlms_action(policy_output["actions_denorm"])
        action_mask = self.policy_head.convert_events_to_action_mask(policy_output["predicted_events"])
        ...
        result = {
            ...
            # RoboVLMs 호환 출력
            "action": robovlms_actions,                             # [B, T, 7] RoboVLMs 포맷
            "action_mask": action_mask                              # [B, T] 액션 유효성 마스크
        }
