...
### 2025-11-14 (최신)
- **대량 데이터 수집 완료** 🎉
  - **총 221개 에피소드 수집 완료** (목표: 1000개, 22.1%)
  - **오늘 수집**: 85개 (1box_left: 54개, 1box_right: 31개)
  - **시나리오별 현황**:
    - 1box_left: 159개 / 250개 (63.6%) 🟢🟢
    - 1box_right: 62개 / 250개 (24.8%) 🟡🟡
    - 2box_left: 0개 / 250개 (0%) ⚪
    - 2box_right: 0개 / 250개 (0%) ⚪
  - **다음 목표**: 1box_right 계속 수집 (목표 100개) + 거리별 다양성 확보 (close/far)

### 2025-11-13
- **미팅용 진행사항 정리** 📋
  - `MEETING_PROGRESS_20251114.md` 작성
  - 1box_left와 1box_right 수집 전략 확인 ✅
  - **결론**: 둘 다 별개 시나리오로 각각 250개씩 필요
  - 권장: 병행 수집으로 균형 유지

- **데이터 수집 진행 중** 📊
  - 총 136개 에피소드 수집 (11월 6일~14일)
  - 자동 측정(A) 반복 모드 정상 작동 확인 ✅
...
- **에피소드 프레임 수 18개로 수정** ✅
  ...
  - 가이드 정규화: 17개 액션으로 변경
  - 복귀 정규화: 17개 액션으로 변경
  - 가이드 편집: 최대 17개까지 입력 가능

- **자동 복귀(B) 정지 신호 최적화** ⚡
  - 복귀 시 정지 신호 최소화로 부드러운 연속 실행
  - 각 액션마다 0.4초만 대기, 추가 정지 신호 제거
  - 타이머 의존 제거로 끊김 없는 이동
  - 예상 소요 시간: 17 × 0.4초 = 6.8초
  
- **자동 측정(A)은 안정성 유지**
  - 데이터 수집의 정확성을 위해 타이머 + 8회 정지 유지
  - 각 액션마다 정지 신호 확실히 발행
  - 안정화 대기 0.3초 유지
