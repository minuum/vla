#!/usr/bin/env python3
"""
🎯 Discrete Action Head 모델 학습 스크립트
"""
...
from enhanced_kosmos2_clip_hybrid_with_discrete_action_head import create_model
import sys
...
def train_discrete_action_head(
    epochs: int = 5,
    batch_size: int = 4,
    learning_rate: float = 1e-4,
    device: str = "cuda" if torch.cuda.is_available() else "cpu",
    save_dir: str = "discrete_action_head_results"
):
    """Discrete Action Head 모델 학습"""
    
    logger.info("🚀 Discrete Action Head 모델 학습 시작")
    ...
    dataset = MobileVLADataset(
        data_dir="data/mobile_vla_dataset",
        max_episodes=72,
        image_size=224,
        action_dim=2
    )
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = save_path / f"best_discrete_action_head_epoch_{epoch+1}.pth"
        ...
        # 에포크별 모델 저장
        epoch_model_path = save_path / f"discrete_action_head_epoch_{epoch+1}.pth"
        torch.save({
            'epoch': epoch + 1,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 최종 모델 저장
    final_model_path = save_path / "final_discrete_action_head.pth"
    torch.save({
        'epoch': epochs,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 학습 히스토리 저장
    history_path = save_path / "training_history_discrete.json"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Discrete Action Head 모델 학습")
    ...
    parser.add_argument("--save_dir", type=str, default="discrete_action_head_results", help="결과 저장 디렉토리")
    
    args = parser.parse_args()
    
    # 학습 실행
    results = train_discrete_action_head(
        epochs=args.epochs,
