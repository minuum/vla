...
from enhanced_kosmos2_clip_hybrid_with_gpt2_action_head import create_model
import sys
...
def train_gpt2_action_head(
    epochs: int = 5,
    batch_size: int = 4,
    learning_rate: float = 1e-4,
    device: str = "cuda" if torch.cuda.is_available() else "cpu",
    save_dir: str = "gpt2_action_head_results"
):
    """GPT2 Action Head 모델 학습"""
    
    logger.info("🚀 GPT2 Action Head 모델 학습 시작")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = save_path / f"best_gpt2_action_head_epoch_{epoch+1}.pth"
        ...
        # 에포크별 모델 저장
        epoch_model_path = save_path / f"gpt2_action_head_epoch_{epoch+1}.pth"
        torch.save({
            'epoch': epoch + 1,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 최종 모델 저장
    final_model_path = save_path / "final_gpt2_action_head.pth"
    torch.save({
        'epoch': epochs,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 학습 히스토리 저장
    history_path = save_path / "training_history_gpt2.json"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GPT2 Action Head 모델 학습")
    ...
    parser.add_argument("--save_dir", type=str, default="gpt2_action_head_results", help="결과 저장 디렉토리")
    
    args = parser.parse_args()
    
    # 학습 실행
    results = train_gpt2_action_head(
        epochs=args.epochs,
