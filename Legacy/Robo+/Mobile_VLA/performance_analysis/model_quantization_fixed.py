...
class FixedModelQuantizer:
    """
    수정된 모델 양자화 클래스
    """
    
    def __init__(self, model_path: str, output_dir: str = "quantized_models_fixed"):
    ...
    def _create_simple_model(self) -> nn.Module:
        """간단한 모델 구조 생성"""
        class SimpleMAE0222Model(nn.Module):
            def __init__(self):
                super().__init__()
    ...
    def quantize_model(self) -> Dict[str, Any]:
        ...
        if ONNX_AVAILABLE:
            ...
            onnx_path = self.output_dir / "mae0222_model_fixed.onnx"
        ...
        # 결과 저장
        results_path = self.output_dir / "mae0222_quantization_results_fixed.json"
    ...
    def _print_results(self, results: Dict[str, Any]):
        """양자화 결과 출력"""
        print("\n" + "="*60)
        print("🤖 Mobile VLA 모델 양자화 결과 (MAE 0.222 - 수정판)")
        print("="*60)
...
def main():
    """메인 함수"""
    print("🚀 Mobile VLA 모델 양자화 시작 (MAE 0.222 - 수정판)")
    ...
    # 양자화 실행
    quantizer = FixedModelQuantizer(model_path)
