# Critical Issue #7: VLM Fine-tuning 목적 분석
...
- **LRN 토큰은 새로 추가된 learnable parameter**
- **VLM fine-tuning의 목적은 LRN 토큰의 의미를 학습**
- **Multimodal fusion을 통해 로봇 제어에 특화된 이해를 얻음**
- **기존 VLM의 지식을 활용하면서 도메인 특화**

### Fine-tuning 과정

1. **LRN 토큰 초기화**: 0으로 초기화된 의미 없는 벡터
2. **VLM 통과**: Self-attention을 통해 이미지+텍스트+액션 정보 융합
3. **의미 학습**: LRN 토큰이 특정 액션을 나타내는 벡터로 학습
4. **액션 예측**: 융합된 정보를 바탕으로 적절한 액션 예측
