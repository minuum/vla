...
5. **에피소드 길이(Frame 수) 다변화**: 각 주행마다 타겟까지의 거리, 속도, 녹화 시작 후 멈춰있는 시간 등을 약간씩 다르게 두어 에피소드의 총 길이(예: 30프레임~80프레임)를 다양하게 수집합니다. 특정 프레임 넘버에 특정 액션이 편중되는 문제를 막기 위함입니다.
...
- **타이밍 암기 원천 차단 효과**:
  - 기존 v2는 1) 항상 일정한 에피소드 길이(프레임 수)를 가졌고, 2) 동일 시점에 액션 변화가 생겼습니다. 
  - Phase 1-2는 프레임 0부터 조향 액션이 랜덤하게 시작될 뿐만 아니라, **에피소드들의 총 길이(프레임 수) 역시 다양하게 섞여 들어가므로**, Frame Number와 Action 간의 상관관계(Spurious Correlation)가 완전히 파훼됩니다. 모델이 '시간'을 바탕으로 액션을 추론하는 것이 원천적으로 불가능해집니다.
