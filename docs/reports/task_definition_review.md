...
> **태스크**: 장애물(박스)을 피해서 목표물(beverage bottle)로 이동
> - 목표물은 **고정** 위치에 있음
> - 중간에 **장애물(박스)**이 있음
> - **Left**: 박스를 **왼쪽으로 돌아서** 피해감
> - **Right**: 박스를 **오른쪽으로 돌아서** 피해감

### Action 시퀀스 분석 (18 프레임)

**LEFT 에피소드 (박스를 왼쪽으로 피해감)**:
```
Frame 0-2:  정지/전진 (시작)
Frame 3-10: 전진 + 왼쪽 (박스 왼쪽으로 피하기)
Frame 11-12: 전진 (박스 옆 지나감)
Frame 13-15: 전진 + 오른쪽 (목표물로 복귀)
Frame 16-17: 전진 (도착)
```

**RIGHT 에피소드 (박스를 오른쪽으로 피해감)**:
```
Frame 0-2:  정지/전진 (시작)
Frame 3-10: 전진 + 오른쪽 (박스 오른쪽으로 피하기)
Frame 11-12: 전진 (박스 옆 지나감)
Frame 13-15: 전진 + 왼쪽 (목표물로 복귀)
Frame 16-17: 전진 (도착)
```
...
| 잘못된 이해 | 올바른 이해 |
|:---|:---|
| 물체가 왼쪽/오른쪽에 위치 | 물체는 **고정**, 회피 경로가 다름 |
| 물체 위치 → action | **언어 지시** → 회피 방향 선택 |
| 이미지로 방향 결정 | **언어 지시문**이 방향 결정 |
