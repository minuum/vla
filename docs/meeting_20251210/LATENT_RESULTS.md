...
**사용한 모델**: **Pre-trained Kosmos-2** (Fine-tuned 아님!)
- ✅ Checkpoint 로딩 실패
- ✅ 대안으로 Pre-trained 모델 사용
- ❌ Fine-tuned weights 미사용

**의미**:
1. **Pre-trained VLM은 Left/Right를 구분 못함** (0.0006 separation)
2. **Fine-tuning이 중요하다!**
3. **LoRA가 실제로 Left/Right를 학습했다는 증거**
...
**"Pre-trained Kosmos-2로 latent space를 분석했습니다"**

**결과**:
- Separation = **0.0006** (거의 0)
- Pre-trained는 Left/Right **구분 못함**

**의미**:
- ✅ **Fine-tuning이 필수!**
- ✅ **우리 LoRA가 실제로 학습했다는 증거**
...
**"Fine-tuned checkpoint로 재분석이 필요합니다"**

**문제**:
- Checkpoint 경로 오류
- Fine-tuned weights 로딩 실패

**해결**:
1. Checkpoint 경로 수정
2. Lightning 모델 로딩 방식 수정
3. Fine-tuned hidden states 추출
...
**"Fine-tuned 모델은 훨씬 높은 separation을 보일 것"**
...
**방금 확인한 것**:
- ✅ **Pre-trained**: Separation 0.0006 (거의 구분 못함)
- ⏳ **Fine-tuned**: 예상 0.3-0.4 (명확한 구분)
...
**"Fine-tuned checkpoint로 재분석하면 명확한 결과를 얻을 수 있습니다"**
